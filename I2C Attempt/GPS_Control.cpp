...
void loop() { // run over and over again
  ...
  if (AnemData = 1) { // normal functioning
    targetValueElevation = Elevation_deg;
...
// Read normalized values from MPU6050
  ...
  // Calculate Solar Panel Elevation from MPU6050
  int elevation = 90 - (atan2(normAccel.XAxis, sqrt(normAccel.YAxis*normAccel.YAxis + normAccel.ZAxis*normAccel.ZAxis))*180.0)/M_PI;

    Serial.print("Elevation_deg: ");
    Serial.println(Elevation_deg);
    Wire.beginTransmission(11);
    Wire.write(targetValueElevation); //Transmit target elevation
    Wire.endTransmission();
    Wire.beginTransmission(11);
    Wire.write(elevation); // Transmit current elevation
    Wire.endTransmission();
