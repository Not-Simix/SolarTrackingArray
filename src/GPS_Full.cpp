...
{ // run over and over again
  ...
  if (millis() - timer > 2000)
  {
    ...
    eqtime = (229.18) * ((.000075) + (.001868 * cos(fract_year_rad)) - (.032077 * sin(fract_year_rad)) - (0.014615 * cos(2 * fract_year_rad)) - (0.040849 * sin(2 * fract_year_rad)));

    decl_rad = (0.006918) - (0.399912 * cos(fract_year_rad)) + (0.070257 * sin(fract_year_rad)) - (0.006758 * cos(2 * fract_year_rad)) + (0.000907 * sin(2 * fract_year_rad)) - (0.002697 * cos(3 * fract_year_rad)) + (0.00148 * sin(3 * fract_year_rad));
    ...
    if (hour < 20 && hour > 5)
    { // if between 5 and 20, function as normal
      Elevation_deg = 90 - Zenith_deg;
    }
    else
    { // if outside 5 to 20, go to 0
      Elevation_deg = 0;
    }
